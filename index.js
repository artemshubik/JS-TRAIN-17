...
function findWordOccurrences(str, word) {
  ...
  // Створення пустого масиву для зберігання індексів входжень
  let matches = [];
  let match;

  // Використання циклу while для отримання всіх збігів регулярного виразу
  ...
  // Повернення масиву з індексами входжень
  return matches;
}
...
function checkRegexFlags(regex) {
  ...
  // Перевіряємо наявність флагів 'g' та 'm' за допомогою методу `includes`
  let hasGFlag = flags.includes("g");
  let hasMFlag = flags.includes("m");
...
function replaceWordOccurrences(str, word, newWord) {
  ...
  // Заміняємо всі входження слова у рядку на нове слово за допомогою методу `replace()`.
  let replacedStr = str.replace(regex, newWord);

  // Повертаємо результат заміни.
  return replacedStr;
}
